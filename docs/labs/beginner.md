...
* [Switch Task](../reference-docs/switch-task.md) that takes output from the previous task, and decides whether to schedule the `add_idents` task.
...
Let's create the [task definition](../configuration/taskdef.md) for `verify_if_idents_are_added` in JSON. This task will be a *SIMPLE* task which is supposed to be executed by an Idents microservice. We'll be mocking the Idents microservice part.
...
And a [responseTimeout](../architecture/tasklifecycle.md#response-timeout-seconds) of 180 seconds.
...
We can define several other fields defined [here](../configuration/taskdef.md), but this is a good place to start with.
...
Learn more about wiring inputs and outputs [here](../configuration/workflowdef.md#wiring-inputs-and-outputs).
...
Checkout the Switch task structure [here](../reference-docs/switch-task.md).
...
Ideally, the workers implementing the [Client](../gettingstarted/client.md#worker) interface would do this process, executing the tasks on real microservices. But, let's mock this part.
