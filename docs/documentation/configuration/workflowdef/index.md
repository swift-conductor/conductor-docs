...
The Workflow Definition contains all the information necessary to define the behavior of a workflow. The most important part of this definition is the `tasks` property, which is an array of [**Workflow Tasks**](#task-configurations).
...
The `tasks` property in a Workflow Definition defines an array of _Workflow Tasks_. This is the blueprint for the workflow. Workflow Tasks can reference different types of Tasks.

- System Tasks
- System Operators
- Custom Tasks

Note: Task Configuration should not be confused with **Task Definitions**, which are used to register CUSTOM (worker based) tasks.
...
| type              | string  | Type of task. CUSTOM for tasks executed by remote workers, or one of the system task types                                                  |                                                         |
...
- Example: In your Workflow Definition, you can define your inputTemplate as:
...
And the default `url` would be `https://some_url:7004` if no `url` was provided as input to your workflow.
